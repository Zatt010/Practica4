Given(/^0I am on the GMO homepage$/) do
  visit 'https://demo.borland.com/gmopost/'
end

When(/^0I click on "Enter GMO Online"$/) do
  find('body > form > div:nth-child(1) > center > table > tbody > tr > td:nth-child(1) > input[type=button]').click
end

Then(/^0I should see the GMO Online Catalog page$/) do
  expect(page).to have_current_path('https://demo.borland.com/gmopost/online-catalog.htm')
end

Then(/^0I should see "These are the items currently available through our online catalog"$/) do
...
Then(/^0I should see "3 Person Dome Tent" with price "\$ 299.99"$/) do
...
Then(/^0I should see "External Frame Backpack" with price "\$ 179.95"$/) do
...
Then(/^0I should see "Glacier Sun Glasses" with price "\$ 67.99"$/) do
...
Then(/^0I should see "Padded Socks" with price "\$ 19.99"$/) do
...
Then(/^0I should see "Hiking Boots" with price "\$ 109.90"$/) do
...
Then(/^0I should see "Back Country Shorts" with price "\$ 24.95"$/) do
